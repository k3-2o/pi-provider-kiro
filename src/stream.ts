...
//
// AWS Builder ID accounts are not authorized for ListAvailableProfiles and
// always receive 403 Forbidden. That is expected — Builder ID has no profile
// concept, so profileArn is simply omitted from the request body downstream
// and generation works fine. To avoid re-hitting the endpoint (and spamming
// the UI with a warning) on every single message, failures are held in a
// negative cache for a cooldown period, after which one retry is attempted.
const profileArnCache = new Map<string, string>();
const profileArnPending = new Set<string>();
const profileArnNegativeCache = new Map<string, number>(); // endpoint -> cooldown expiry ms
const PROFILE_ARN_NEGATIVE_COOLDOWN_MS = 10 * 60 * 1000; // 10 minutes
...
async function resolveProfileArn(accessToken: string, endpoint: string): Promise<string | undefined> {
  ...
  // Negative cache: a recent failure means ListAvailableProfiles is unsupported
  // for this account (e.g. AWS Builder ID). Skip silently to avoid per-message
  // fetch overhead and UI warning spam. One retry is allowed after cooldown.
  const negExpiry = profileArnNegativeCache.get(endpoint);
  if (negExpiry && Date.now() < negExpiry) {
    debugLog("profileArn.skipped", {
      message: "Skipping profileArn resolution (in negative cooldown after prior failure; expected for AWS Builder ID).",
    });
    return undefined;
  }
  if (profileArnPending.has(endpoint)) return undefined;
  const firstFailure = !negExpiry;
  try {
